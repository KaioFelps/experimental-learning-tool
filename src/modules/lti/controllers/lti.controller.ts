...
import { ACCESS_TOKEN_SESSION_KEY, LTI_TOKEN_SESSION_KEY, XSRF_SESSION_KEY } from "lti/src";
...
export class LtiController {
  ...
  async launch(@Req() request: HttpRequest, @Res() response: HttpResponse) {
    ...
    if (either.isLeft(launch)) throw new BadRequestException(treeifyError(launch.left));

    const { id_token: idToken, state } = launch.right;

    checkXsrfOrThrow(request.session[XSRF_SESSION_KEY], state);

    const tokenData = await this.ltiService.decodeAndVerifyIdToken(request, idToken);
